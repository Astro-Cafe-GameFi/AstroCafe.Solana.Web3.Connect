...
function App() {
  ...
  async function signMessage(message) {
    ...
    if (signature) {
      ...
      if (message === "Please sign this message to log in with your wallet address.")
        displayResponse("Signature complete.\n\nCopy to clipboard and sign the next message to login", `${account}-${signature}`);
      else
        displayResponse("Signature complete.\n\nCopy to clipboard and return to Astro Café", `${account}-${signature}`);
    } else {
      displayResponse("Signature Denied.\n\nCopy to clipboard and return to Astro Café", "error");
    }
  }
  ...
  async function sendRawTransaction(txMessage) {
    ...
    if (txHash) {
      console.log({ txHash });
      displayResponse("Transaction sent.\n\nCopy to clipboard and return to Astro Café", txHash);
    } else {
  ...
  return (
    <div id="app">
      <div className="header">
        <button id="disconnect-button" title="Disconnect wallet" className={showDisconnectButton ? 'active': ''} onClick={() => reset()}>X</button>
      </div>
      <div className="center">
        <div id="logo">
          <img src={logo} alt="logo" />
        </div>
        <div id="response-text" className={showText ? 'active': ''}>{text}</div>
        <button id="response-button" className={showButton ? 'active': ''} onClick={() => copyToClipboard(response)}>{buttonText}</button>
      </div>
    </div>
  );
}
